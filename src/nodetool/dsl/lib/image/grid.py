...
class CombineImageGrid(GraphNode):
    ...
    tiles: list[types.ImageRef] | GraphNode | tuple[GraphNode, str] = Field(default=[], description='List of image tiles to combine.')
...
class SliceImageGrid(GraphNode):
    ...
    image: types.ImageRef | GraphNode | tuple[GraphNode, str] = Field(default=types.ImageRef(type='image', uri='', asset_id=None, data=None), description='The image to slice into a grid.')
    columns: int | GraphNode | tuple[GraphNode, str] = Field(default=0, description='Number of columns in the grid.')
